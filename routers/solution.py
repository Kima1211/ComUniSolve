...
def update_solution(solution_id: int, db:Session=Depends(get_db),current_user: user.User=Depends(get_current_user)):
    ...
    if fnd_problem.status == "resolved":
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="A solution has already been accepted for this problem")
